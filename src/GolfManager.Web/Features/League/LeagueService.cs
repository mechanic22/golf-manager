...
public class LeagueService : ILeagueService
{
    ...
    public async Task<ApiResponse<List<LeagueResponse>>?> DiscoverLeaguesAsync(string? search = null)
    {
        try
        {
            var url = "api/v1/leagues/discover";
            if (!string.IsNullOrWhiteSpace(search))
                url += $"?search={Uri.EscapeDataString(search)}";

            return await _httpClient.GetFromJsonAsync<ApiResponse<List<LeagueResponse>>>(url);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error discovering leagues");
            return null;
        }
    }

    public async Task<ApiResponse<List<LeagueResponse>>?> GetAllLeaguesAsync(string? search = null)
    {
        try
        {
            var url = "api/v1/admin/leagues";
            if (!string.IsNullOrWhiteSpace(search))
                url += $"?search={Uri.EscapeDataString(search)}";
        ...
        {
            _logger.LogError(ex, "Error fetching all leagues (admin)");
            return null;
        }
    }
    ...
    public async Task<ApiResponse<GuestStandingsResponse>?> GetGuestStandingsAsync(string leagueKey)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                return await response.Content.ReadFromJsonAsync<ApiResponse<GuestStandingsResponse>>();
            }

            var errorContent = await response.Content.ReadAsStringAsync();
            _logger.LogError("Failed to get guest standings. Status: {StatusCode}, Error: {ErrorContent}", response.StatusCode, errorContent);

            if (!string.IsNullOrEmpty(errorContent))
            {
                try
                {
                    var errorResponse = await response.Content.ReadFromJsonAsync<ApiResponse<GuestStandingsResponse>>();
                    if (errorResponse != null)
                    {
                        return errorResponse;
                    }
            ...
            return ApiResponse<GuestStandingsResponse>.ErrorResponse(
                $"Request failed with status {response.StatusCode}",
                errorContent);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Exception getting guest standings");
            return ApiResponse<GuestStandingsResponse>.ErrorResponse("Request failed", ex.Message);
        }
    }
}
