...
public class RoundService : IRoundService
{
    ...
    public async Task<List<RoundResponse>> GetGolferRoundsAsync(string leagueId, string golferId)
    {
        try
        {
            var response = await _httpClient.GetFromJsonAsync<ApiResponse<PagedResponse<RoundResponse>>>(
                $"api/v1/rounds/golfer/{golferId}?pageSize=100");

            return response?.Data?.Items ?? new List<RoundResponse>();
        }
        catch (Exception ex)
        {
