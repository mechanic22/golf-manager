...
public class UserService : IUserService
{
    ...
    public async Task<ApiResponse<GolferStatsResponse>?> GetMyStatsAsync()
    {
        try
        {
            return await _httpClient.GetFromJsonAsync<ApiResponse<GolferStatsResponse>>("api/v1/users/me/stats");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error fetching golfer stats");
            return ApiResponse<GolferStatsResponse>.ErrorResponse("Failed to load stats", ex.Message);
        }
    }

    public async Task<ApiResponse<UserProfileResponse>?> UpdateCurrentUserAsync(UpdateProfileRequest request)
    {
        try
        {
            var response = await _httpClient.PutAsJsonAsync("api/v1/users/me", request);
            if (response.IsSuccessStatusCode)
            {
                return await response.Content.ReadFromJsonAsync<ApiResponse<UserProfileResponse>>();
            }
        ...
        catch (Exception ex)
        {
            ...
            return ApiResponse<UserProfileResponse>.ErrorResponse("Failed to update profile", ex.Message);
        }
    }
