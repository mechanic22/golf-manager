...
public partial class Admin : ComponentBase
{
    ...
    private async Task LoadData()
    {
        ...
        try
        {
            ...
            // TODO: Load real activity logs from API
            // For now, keeping placeholder activity logs
            recentActivity = new List<ActivityLog>
            {
                new() { Type = "user", Description = $"{stats.NewUsersThisMonth} new users this month", Timestamp = DateTime.Now.AddHours(-1) },
                new() { Type = "league", Description = $"{stats.TotalLeagues} total leagues in system", Timestamp = DateTime.Now.AddHours(-2) },
                new() { Type = "event", Description = $"{stats.UpcomingEvents} upcoming events", Timestamp = DateTime.Now.AddHours(-5) },
            };
        }
        catch (Exception ex)
        {
