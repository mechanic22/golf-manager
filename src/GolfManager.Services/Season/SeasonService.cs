...
public class SeasonService : ISeasonService
{
    ...
    public async Task<PlayerSeasonHoleStatsResponse?> GetPlayerSeasonHoleStatsAsync(string seasonId, string leagueGolferId, string leagueId)
    {
        // Find all season events for this season/league to identify which rounds belong to this season
        var seasonEvents = await _context.SeasonEvents
            .IgnoreQueryFilters()
            .Where(e => e.SeasonId == seasonId && e.LeagueId == leagueId)
            .Select(e => new { e.EventDate, e.CourseId, e.TeeId })
            .ToListAsync();

        if (seasonEvents.Count == 0)
            return new PlayerSeasonHoleStatsResponse();

        // Get all rounds for this league golfer with their hole scores
        ...
        // Filter rounds that match a season event (by date + course + tee)
        var seasonRounds = rounds.Where(r =>
            seasonEvents.Any(e =>
        ...
        // Load HoleTees for all unique tees used in season rounds
        var teeIds = seasonRounds.Select(r => r.TeeId!).Distinct().ToList();
